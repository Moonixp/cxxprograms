...
void URIParser::parse_userinfo_host_port(const std::string &authority,
  ...
  size_t colon_pos = authority.find(':', at_pos + 1);
  ...
  size_t host_colon_pos = host.find(':');

  if (host_colon_pos != std::string::npos) {

    std::string port_str = host.substr(host_colon_pos + 1);
...
/**
 * Parse the path component of a URI from an input stream
 *
 * Read the path component of a URI from an input stream and store it in the
 * given URI object. Parsing stops when a '?' or '#' character is encountered,
 * or when the end of the stream is reached.
 *
 * @param input The input stream to read from
 * @param uri The URI object to store the parsed path in
 * @return true if the path component was successfully parsed, false otherwise
 */
bool URIParser::parse_path(std::istream &input, URI &uri) const {
  ...
  /**
   * Parse the query component of a URI from an input stream
   *
   * Read the query component of a URI from an input stream and store it in the
   * given URI object. Parsing stops when a '#' character is encountered, or
   * when the end of the stream is reached.
   *
   * @param input The input stream to read from
   * @param uri The URI object to store the parsed query in
   * @return true if the query component was successfully parsed, false otherwise
   */
bool URIParser::parse_query(std::istream &input, URI &uri) const {
  ...
  /**
   * Parse the fragment component of a URI from an input stream
   *
   * Read the fragment component of a URI from an input stream and store it in the
   * given URI object. Parsing stops when the end of the stream is reached.
   *
   * @param input The input stream to read from
   * @param uri The URI object to store the parsed fragment in
   * @return true if the fragment component was successfully parsed, false otherwise
   */
